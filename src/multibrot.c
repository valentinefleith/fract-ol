...
/*   Updated: 2024/02/27 21:23:55 by vafleith         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	calculate_multithree(t_pixel px, t_fractal *fractal)
{
	...
	{
		z.imaginary = (3 * r_square * z.imaginary - z.imaginary * z.imaginary * z.imaginary) + point.imaginary;
...
static int	calculate_multifour(t_pixel px, t_fractal *fractal)
{
	...
	while (i < fractal->max_iterations)
	{
		z.imaginary = (4 * r_square * z.real * z.imaginary -  4 * z.real * z.imaginary * z.imaginary * z.imaginary) + point.imaginary;
		z.real = r_square * r_square - 6 * r_square * i_square + i_square * i_square + point.real;
		r_square = z.real * z.real;
		i_square = z.imaginary * z.imaginary;
		if (r_square + i_square > MAX)
			return (get_color(i, fractal));
...
static int	calculate_multifive(t_pixel px, t_fractal *fractal)
{
	...
	while (i < fractal->max_iterations)
	{
		z.imaginary = (5 * r_square * r_square * z.imaginary -  10 * r_square * i_square * z.imaginary + i_square * i_square * z.imaginary) + point.imaginary;
		z.real = r_square * r_square * z.real - 10 * r_square * i_square * z.real + 5 * z.real * i_square * i_square + point.real;
		r_square = z.real * z.real;
		i_square = z.imaginary * z.imaginary;
		if (r_square + i_square > MAX)
			return (get_color(i, fractal));
